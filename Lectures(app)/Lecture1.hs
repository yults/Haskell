...
--   статический(не динамический), с сильной типизацией, иммутабельный (не именяемые переменные)
...
--  и то что в in может присвоится значению функции а where - просто отдельный блок
-- Let -----------
...
collatzSum n
  ...
  | otherwise = n + collatzSum (3 * n + 1) -- otherwise это true
-- Case ------------------------------
getFont :: Int -> String
getFont n = case n of
                0 -> "PLAIN"
